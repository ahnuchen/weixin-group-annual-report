...
export function getPerEmojiCount(text:string) {
    const countMap: Record<string, number> = {}
    // 循环遍历替换规则
    for (let i = 0; i < replacementRules.length; i++) {
        const rule = replacementRules[i];
        const emojis = text.match(rule.pattern) ?? []
        for (const emoji of emojis) {
            if(!countMap[emoji]) {
                countMap[emoji] = 1
            } else {
                countMap[emoji] = countMap[emoji] + 1
            }
        }
    }
    return countMap;
...
export function countEmoji(text: string) {
    ...
    for (let i = 0; i < replacementRules.length; i++) {
        const rule = replacementRules[i];
        count += (text.match(rule.pattern)?.length ?? 0)
    }
    return count;
}